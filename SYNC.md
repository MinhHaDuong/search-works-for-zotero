...
have a reachable choice. The maintainer confirmed the issue's source reading in
full; the author then closed it as completed. It removes the measured cause of
goal 1's
`R10-no-egress` failure. Ticket 0670 subsequently re-read all twenty-three
requirements at `34d6c26`, two documentation commits past the v1.14.0 tag
(`verification/UPSTREAM-1.14.0-REREAD.md`). The source verdicts do not move;
the valid smoke rerun keeps R6, R10, R15 and R23 measured
(`bench/results/smoke-1.14.0/checks.json`). The executable-baseline acceptance
runner could not be admitted on this host without widening access to the
author's private Node installation, so its failed launch is not evidence and
is not committed. R3 and R13 are consequently downgraded from `measured` to
`code` until that layer can be rerun on an admissible substrate.
...
| #54 update check default egress | filed by **us**, 2026-09-04, after the author's explicit go-ahead: [oscardvs/zoteus#54](https://github.com/oscardvs/zoteus/issues/54); **closed COMPLETED** the same day after the maintainer built it in `489b24b` and shipped it in **v1.14.0**. He took the proposed default-off shape and added the missing desktop-bundle surface: `ZOTEUS_UPDATE_CHECK` now defaults `false`, while `mcpb/manifest.json` exposes a **Check for updates** switch, also off by default. His closing account confirms the filed source reading in every particular: the old default was `bool(true)`, startup made one unauthenticated GET to `api.github.com`, the no-cloud-key path did not reach `api.zotero.org`, no identifying or library content travelled, and the defect was the unasked-for request itself. Goal 1's measured failure cause is removed in source; `R10-no-egress` is remeasured only with the v1.14 baseline reread, not promoted from this source result. Draft: `verification/ISSUE-DRAFT-0634.md`; ticket 0634. |
...
| Zotero core | tracked as a surface per DECISIONS.md 2026-08-28. [zotero/zotero#6012](https://github.com/zotero/zotero/pull/6012) — dstillman's **draft** semantic-search PR (opened 2026-08-05, last activity 2026-08-26): local ONNX models via Firefox's runtime, RRF hybrid ranking, **sqlite-vec** for vectors — the same extension #30 proposes for zoteus. Added to the watch 2026-08-29 at the author's request: [zotero/zotero#1610](https://github.com/zotero/zotero/issues/1610) — dstillman's issue (2018-12-19, **closed**), "Undelete and overwrite items in target library trash for repeat cross-library copy", part of the cross-library overhaul around zotero#140; trash/undelete behavior on repeat cross-library copies sits under what the local API serves group-library indexing (#12). The rendered page does not surface the close date or closing commit — verify before citing either. **Checkpoint read at source 2026-08-30** (clone of `zotero/zotero`, PR head fetched as `refs/pull/6012/head`), discharging the political-review gate carried by tickets 0025, 0028, 0034 and 0024's I-3: #6012 is still a draft at head `77e2c4b` — the same commit `SPEC.md` read on 2026-08-29, so it has not moved — and is not an ancestor of `main` (tip `bccaf46`), merge-base `3af8cea`. It adds `embeddings.js`, `lexical.js`, `bestMatch.js` and `ml.js` and modifies `fulltext.js` and `sdt.js`, and **touches no `server/`, API or connector file at all**, which confirms at source what FIELD-REVIEW established by page fetch. The load-bearing fact is separate from the PR: `sdt.js` already ships on `main`, so structured extraction is in the application today, yet `server_localAPI.js` carries zero `sdt`/`structured` references — the only text surface is flat `/fulltext`, read from `Zotero.Fulltext.getItemCacheFile()`. Structured extraction therefore does not reach the consumer zoteus is, merge or no merge. One mechanism confirmed for `SPEC.md` C2's last bullet: the PR adds a `bestMatch` search *condition*, and the local API already serves `/searches/:searchKey/items`, so on merge a saved search carrying that condition is the first place platform semantic results reach the API, with no new endpoint. Re-open the checkpoint on a structured-text endpoint, not merely on a merge. **Drift signal, added 2026-09-02 (ticket 0060): `fulltext.js` and `resource/schema/userdata.sql` on `main`, not the PR.** Zotero 10's largest recent move inside our own problem area — dropping `fulltextWords`/`fulltextItemWords` at userdata step 127 and splitting the keyword index into a separate attached `fulltext.sqlite` of four contentless FTS5 tables — shipped **unannounced**: the 10.0 changelog says only "Much faster full-text content searches", and it reached this repository through a third party's draft PR rather than through any release note. Both files carried it (`7c2a1d1`, 2026-06-30), so watching them would have fired on that date; watching releases did not fire at all. **#6012 HAS MOVED since the 2026-08-30 checkpoint, read on the PR's commit page 2026-09-03**: four commits dated **2026-09-02** — `cb27b75` "run all sdt extraction before any embedding begins", `fbb0495` "approximate chunk token sizes instead of tokenizer", `19e7962` "more polite memory and cpu usage", `cb178d9` "minor fixes" (and `d1f1391`, 2026-08-28, "use sqlite_vec extension to speed up vector search"). The "last activity 2026-08-26" and "has not moved" statements above are the readings of their own dates and are left standing; this sentence supersedes them. `cb27b75` is the load-bearing one: it makes SDT extraction a phase that completes **before** any embedding begins, which is the mechanism that would make packs library-wide — ticket 0572's deferral checkpoint — so that checkpoint is nearer than 0572's text assumes, and `19e7962` puts their extraction scheduling under active change. Read on the rendered commit list, not at source: the head SHA, the draft state and whether `server_localAPI.js` is still untouched are **not** re-verified here, and the 2026-08-30 at-source checkpoint remains the last verified one. **#6012 HAS MOVED AGAIN, read at source 2026-09-12 (ticket 0754): head `4427be9a`, seventeen commits past `19e7962`, dated 2026-09-12.** The readings above are the readings of their own dates and stand; this sentence supersedes them on the head, and `verification/BRIDGE-0496.md` and `verification/VERIFY-FULLTEXT-SQLITE.md` stay pinned at `19e7962` as dated evidence rather than being rewritten. Four things moved, three in `chrome/content/zotero/xpcom/embeddings.js`. (1) **Queue order reversed in kind.** `_getEligibleItemIDs()` no longer orders attachments smallest first — that comment and its head-of-line rationale are gone — and now orders them most-recently-touched first (`ORDER BY MAX` over `lastRead`, the attachment's `dateModified`, its parent's, its annotations' and its sibling notes', descending), with attachments enqueued after metadata as "an order of magnitude more to index". A cost-minimizing order replaced by a value-maximizing one; `VERIFY-FULLTEXT-SQLITE.md` §5.3 is the superseded reading. (2) **The thread halving is conditional.** Still half `Zotero.ML.getOptimalConcurrency()`, but inside `Zotero.Embeddings.Indexing.getEngineThreads()`, liftable by a `_threadBoosts` set that now includes `user-idle`; the quoted "trade wall time for heat" comment is gone from the source, so `BRIDGE-0496.md`'s quotation is a reading of `19e7962` and not of today. (3) **A new memory mechanism beside the floor**: an inference-process footprint cap above which the engine is restarted between batches, and under memory pressure the token budget halves to a floor with the engine shut down alongside and restored on `memory-pressure-stop`. The 1,5 GiB start floor, its five-minute retry with the queue untouched, and the unreadable-memory case returning 0 and reading as room to proceed are all unchanged — the read moved from a private `_availableMemory()` to a public `Zotero.Embeddings.Diagnostics.getAvailableMemory()`. (4) **The outbound passage-embedding hook is now a subsystem**: `Zotero.Embeddings.Endpoint` (`getActive`/`getSentinel`/`checkBatch`/`recordFailure`/`recordSuccess`/`recheck`/`getStatus`) with a sentinel-vector check that the endpoint really serves embeddings, plus a user-facing `chrome/content/zotero/preferences/embeddingsEndpoint.xhtml` (verified present at that ref). Unchanged and re-verified at the new head: the 5 s post-startup `startIndexing()`, the 3 s enqueue debounce, the `shouldStop()` check before `Zotero.SDT.ensure()` with the graceful between-documents stop, `stopIndexing()` clearing both queues and the kick timer and persisting `embeddings.indexingPaused`, and no GPU device or execution-provider selection anywhere in `embeddings.js`. `test/tests/sdtTest.js` is byte-identical across the two heads (blob `2609c641`), so the `ensure()` stale-processor reading in the extraction row below needs no re-derivation. Not re-verified here: the draft state, and whether `server_localAPI.js` is still untouched |
| Zotero extraction | tracked as a surface 2026-08-29, on the standing of Zotero core above. Added because the SDT pack is the strongest candidate substrate for the entry-level unit of answer (ticket 0120's third note) and nothing here watched it. [zotero/document-worker](https://github.com/zotero/document-worker) produces the pack, through an ONNX block-segmentation pipeline (clusterer, repair model, classifier) run under ONNX Runtime WASM: 108 commits in 2026, a steady 15/month through July and August, last push 2026-08-19. Its August run targets what looked weak on this machine — "Recover outlines from printed contents" (08-14), then a reference and citation-parsing series — though "weak" rests on **two packs**, which is a probe and not a benchmark: heading recall was 7 of 7 sections on a 1976 scan and 4 headings on a modern 24-page paper, and those two disagree. [zotero/structured-document-text](https://github.com/zotero/structured-document-text) is the pack format itself, a submodule of the first; quiet since 2026-07-29 (2 commits in July, none since), so the schema is settling while the extractor keeps improving, which is the good shape for a reader. **Drift signal, updated 2026-09-11 (ticket 0754): the three version constants in `resource/document-worker/metadata.json`** — now `SDT_PACK_VERSION` 1, `SDT_SCHEMA_VERSION` 1.2.0, `SDT_PROCESSOR_VERSIONS` pdf 14 / epub 2 / snapshot 1, read from installed build 20260909184950 (Zotero 10.0.2). Previously `SDT_SCHEMA_VERSION` 1.1.0, pdf 3 / epub 1 / snapshot 1, build 20260817151751 (Zotero 10.0, 2026-08-17): the schema and the PDF processor both moved across the three point releases between the two readings. **The three constants do not split two ways — they split three, and the middle case is the silent one (corrected 2026-09-11, ticket 0765).** A `SDT_PACK_VERSION` bump, or a schema *major*, changes what a reader must handle AND invalidates stored packs: `sdt.js` validates on `(packVersion, schemaMajorVersion, source.hash, processor.type, processor.version)`, and #6012's suite regenerates on "an incompatible schema major version". A `SDT_PROCESSOR_VERSIONS` bump invalidates without changing the format, and does not always stay in the background — `getPack()` returns the stale pack immediately and regenerates behind it, but `ensure()`, the call the sitter's own census makes, blocks until the fresh pack exists (#6012's `sdtTest.js`, "should regenerate a stale-processor pack before resolving ensure()"). A schema *minor* does NEITHER: validity compares `parseInt(SDT_SCHEMA_VERSION)`, so 1.1.0→1.2.0 invalidates nothing, announces nothing, and still changes what is written — measured on the outline entry, where `level` gave way to `source`/`target` with a resolved `{position: {pageIndex, rect}}` or `url`, across 400 packs per bucket with neither key crossing (`verification/probes/sdt_schema_census.py outline`). Consequence on disk, and it is permanent rather than transitional: `snapshot`'s processor version did not move, so those packs never regenerate and keep the old outline shape beside pdf packs draining to the new one — 13 709 packs on 2026-09-11, 12 774 at 1.1.0 and 935 at 1.2.0 (`… census`). Quality still moves under us without any signal, and now costs a blocking, library-wide pass at a cadence neither upstream repo announces. **Reading the three constants answers what the NEXT pack will be written as, never what is on disk; the census probe is the population reading, and this row needs both.** The commit-cadence reading of 2 document-worker commits behind and 0 schema commits, taken 2026-09-02, has not been refreshed against this build. Neither repo publishes release notes or a roadmap, and its 11 open issues are stale dependabot bumps from 2022–2023, so commit cadence is the only available signal. That is the same blindness that hid the FTS5 split, and the reason this row exists |
