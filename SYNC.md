# SYNC — the fork against upstream v1.12.0
...
the maintainer and closed, two releases shipped in one day. **Nothing of
ours is in flight.** The reviewed baseline in `UPSTREAM` still names
v1.10.0 and is now two releases and one search-layer rewrite stale;
re-basing it, and re-reading the standing it dates, is ticket 0501. The
...
**Status, 2026-08-31** (one table; earlier states are in git history).
...
| fork | `main` at `bb414df`, now **four releases behind** upstream `b05ed69` — re-align when convenient; `cross-library-guard` (`ae6b043`) is **merged upstream** in v1.11.0 and joins `fuse-cosine-loop` (`999cb1c`), `model-cache-under-datadir` (`998865e`) and `gemini-key-header` (`b6312e4`) as deletable by the author (session proxies cannot delete branches — 403 — so this step is always his); `own-words-notes-annotations` (`50245d3`) was never offered as a PR and is **superseded** by his own build of #33, so it goes the same way; `stopwords-follow-up` (`ab89bbc`, correcting the `94d994d` this row carried since 2026-08-28 — ticket 0014 superseded it on 2026-08-29) stays one commit atop `309204b`, now four releases behind and held by X2's verdict; historical storage tree preserved as `archive/fts5-storage-2026-08-21` at `bae82a7` |
