...
| | |
|---|---|
| upstream v1.15.0 | **reviewed at release commit `037bba8`, 2026-09-07**. The `34d6c26..037bba8` range is 24 commits over 45 files, +1495/-292; index schema stays generation 2. Release notes name our uninstall documentation (PR #55) and durable pause (PR #57), both merged 2026-09-06. The full source range also contains citation style/locale forwarding with renamed-style fallback (PR #58), and local embedding inference in a worker thread plus optional `ZOTEUS_LOG_FILE` output (PR #59). Ticket 0735 re-read only the requirements invalidated by those mechanisms; `verification/UPSTREAM-1.15.0-REREAD.md` owns the evidence and explicitly does not carry v1.14 measurements across changed paths. |
