# SYNC — the fork against upstream v1.10.0
...
already implied. PR #32 is the only PR in flight; issues #33 and #34, both
filed from here 2026-08-29, stand open and unanswered. The
...
**Status, 2026-08-30** (one table; earlier states are in git history).
...
| PR #32 wipe guard | **open**, filed 2026-08-29 from `cross-library-guard` (`ae6b043`) — ticket 0016's PR-3, the first item to spend a slot freed by the v1.10.0 sweep. Rebased from its `bb414df` base onto `b132f2d`: five conflicts, all in the files v1.10.0 rewrote, all resolved as additive except the build prologue, where v1.10.0's resume branch owns the reset decision and only the stamp survives. **The hazard grew a second shape in v1.10.0, and the PR now argues both.** `resumeFrom()` gates on `fresh`, the checkpoint, a non-empty store and the embedder — never on the library — so a build for another library against an *interrupted* index skips `clearStore()` and appends its items to the rows already there. Measured on stock v1.10.0: `state: done`, `resumedFrom: 100`, `passages: 102`, `itemsTotal: 2` — one file holding two libraries, its counters describing whichever library asked last. `offsetStillHolds()` cannot catch it; by its own comment it invalidates the crawl offset and keeps the committed rows either way. One `assertLibrary` ahead of `resumeFrom()` covers both shapes. Ten tests, **all ten fail on stock v1.10.0 and pass here**; typecheck, lint, build clean, 857 passed / 7 skipped |
| issue #33 own words | **open**, filed 2026-08-29 — ticket 0022's PR-10, sent as an **issue rather than a PR** and spending no slot. Leads with the asymmetry v1.10.0 created: `zotero_annotate` writes annotations that no query can find, because every crawl is `top: true` and neither a child note nor an annotation is a top-level item. Carries two local-API facts probed here — the boolean `itemType=note || annotation` filter works, so a census costs a page per hundred children rather than a `/children` request per item; and an annotation names the attachment it sits on, never the parent, so attribution needs an `itemKey=` second hop. A prototype is built and pointed at, not offered: fork branch `own-words-notes-annotations` (`50245d3`), source label widened to `'fulltext'|'note'|'annotation'` with no schema change, passages keyed to the PARENT item so `query()`'s per-item dedup keeps own words extending rather than diluting; gates re-verified here independently (typecheck, lint clean, 865 passed / 7 skipped), and nine of its tests fail on stock v1.10.0 as blindness rather than as missing counters. **The form changed because the build came out design-sized**: 17 files, +1050/-22, against +10 / +6 / +77 / +368 for the four contained PRs merged or in flight, and it decides two things the maintainer owns — own words on by default, which changes every user's index size, and whether they ride the full-text pass. The asymmetry the train's form rests on now reads five for five on design-sized work built by him (the SQLite backend, incremental updates, local extraction, resume, the two-stage search) |
| issue #34 migration path | **open**, filed 2026-08-29 from here at 16:44 — an issue, spending no slot. The search index has no migration path: `sidelineIfIncompatible()` accepts exactly two states, `fresh` or the current `SCHEMA_VERSION`, and moves anything else aside for a full rebuild, re-embedding included — an index stamped with an *older* version of the same schema included. The exposure is entirely prospective: `SCHEMA_VERSION` has been `1` since `eee1000` (v1.7.0) and has never been bumped, and pre-backend users persisted to `search-index.json`, a filename this path never inspects. Reproduced on v1.10.0 against a 93 022-passage index in both directions — stamp `0` and stamp `9999` produce the same sideline, byte-for-byte on the moved file. Proposes three graduated remedies: carry `passages.vector` across a rebuild, keyed by passage id and invalidated on `embedderId` change; an additive `if (stored < N) ALTER TABLE` ladder with the sideline as fallback; failing both, price the re-embedding in the status notice before starting it. Ticket 0441 already leans on it — a facet-column schema change should land *with* a migration rather than before one |
| the train | **one in flight** (#32, filed 2026-08-29); #27/#28/#31 all merged the same day. The contained-PR budget's live remainder is **one**: the STOPWORDS follow-up (0014), and it is **held by its own evidence** — X2 measured 2026-08-29 and failed (warm p95 1 773,0 ms stopword-less against 392,3 ms on stock), so per DESIGN §3 the deletion does not ship alone and df-driven pruning (0090/0091) became a precondition rather than a follow-up. PR-3 has now opened as #32, so the six ratified contained PRs stand at four merged (#25, #27, #28, and #19/#20 before them), one sunset-closed (PR-2, overtaken by v1.7.1's `80f8aa0`), one in flight (#32) and one held (0014). The reserve is therefore the live source of further candidates, and its warm-batch condition remains met: 0022 went up 2026-08-29 as issue #33 rather than as a PR, leaving 0019 (terminal no-text state) as the one reserve item still unspent — its warn-once verified still live at `fulltext-source.ts:180`, though #29's local extraction changed the population under it, so its evidence needs re-deriving before it is argued |
