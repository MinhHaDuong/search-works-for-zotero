...
- That window opens on a **"Pause indexing"** checkbox. It is not asked, it is not remembered, and it never inverts: checked means nothing is being indexed, unchecked means something is. The line beside it always says which state you are in and why.
- **Checking it stops new work, and lets the document already under way finish.** It is not an instant stop and cannot be: once an attachment has been handed to Zotero's shared extractor, the plugin has no way to end it. The line beside the box says so while that is true, and points you at the one control that does stop everything — disabling the plugin in Zotero's Add-ons window.
- The box checks itself, and goes grey, while your choice could not change anything anyway: while Zotero is still starting, and while a sync is in progress. It stays *unchecked* and usable while the plugin is merely waiting on memory, disk, processor or Zotero's own indexer — you have not paused anything there, and the line names the condition.
