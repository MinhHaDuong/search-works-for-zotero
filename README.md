...
state where a row could not be anything but a claim about nobody. The committed
artifact under `bench/results/smoke-1.12.0/` predates the two newest checks — it
carries no R22 row and its `assertions_never_seen_red` is that older run's — and
it is regenerated by that target on a machine carrying an isolation mechanism,
which this rung's own R10 clause needs and nothing else here does. It is left as
it stands rather than overwritten by a partial run: an artifact measured where
half the instrument is missing is worse than one that is visibly older.
