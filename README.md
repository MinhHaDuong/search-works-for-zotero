...
| | promise | designed | delivered | evidence | standing |
|---|---|---|---|---|---|
| R19 | Every token the query side produces MUST be one the index side can also produce | ratified | partial | measured | The property holds: the fold merged upstream, and the sweep passed when it ran. The cadence clause — that the sweep runs on every check — left the sheet on 2026-08-31, because what verifies a promise is not itself a promise. It is a gate in SPEC.md §5.2.8, and ticket 0026 wires it. |
