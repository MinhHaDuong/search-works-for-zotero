...
def declaration(arena: Path, *, port: int = 23519) -> Declaration:
    ...
    return Declaration(
        ...
        query_transport=(
            ...
            "not enabled' — measured on /api/, on /api/users/0/items with q= and with "
            "q=+qmode=everything, on /api/users/0/settings and on POST "
            ...
            "xpcom/server/server_localAPI.js:249. The eight probes and their bodies are "
            "in bench/results/0583-zotero-6012/http-surface.json.\n\n"
        ...
        default_configuration=(
            ...
            "shutdown the profile carries 36 preferences in 45 lines — the file's nine "
            "standard header comments make up the difference, and a reviewer was right "
            "to catch an earlier version of this sentence calling all of them "
            "preferences. They are the toolkit's own first-run bookkeeping, the "
            "application's "
            ...
            "writes sixteen of its own keys and whose effective set differs from its "
            "shipped file.)\n\n"
            "How the process is stopped changes that count, which is why the flush is "
            "described rather than assumed: an earlier arm here that killed the process "
            "group after a 12 s grace left ONE preference in the profile, the harness's "
            "port, against 36 after the escalation `process` describes. Same class as "
            "the "
            "residue that appears only seconds after a call returns on the zotero-mcp "
            "seat — a short stop measures a different state, not a cleaner one.\n\n"
        ...
        process=(
            ...
            "so it discloses rather than changes.\n\n"
            "THE PORT IS A DEVIATION FROM THE SHIPPED DEFAULT, DISCLOSED. Not "
            "not-a-deviation: the shipped value is 23119 (same file:172) and this run "
            "used another, and the ratified contract forbids a non-default option with "
            "no carve-out for one that does not touch the feature under test. Three "
            "things belong beside that admission rather than an argument that talks it "
            "away. (a) Why: several instances share this machine, the shipped port is "
            "held by a resident one, and a collision makes the target fail to start — "
            "which the layer would read as a defect of the target rather than of the "
            "arrangement. (b) What it could and could not move, in a form a reader can "
            "CHECK rather than take on trust: no assertion that ran against any adapter "
            "on this roster reads anything through an HTTP port. "
            "check_local_by_default needs configure and status; "
            "check_model_cache_under_declared_roots needs install, query and status; and "
            "every adapter that could have reached a port declares query and status "
            "absent, so those clauses report not-offered before any transport is "
            "touched. The committed artifacts carry the declarations that make that "
            "checkable. (c) The alternative that would remove it, named rather than "
            "omitted: run one instance at a time on the shipped port. That is strictly "
            "more faithful and it costs serialization across lanes, which is why it was "
            "not taken on a machine carrying four; a single-lane run should prefer it. "
            "The deviation came from one lane briefing and appears on three adapters, "
            "so whether the contract's letter should carry a carve-out for coexistence "
            "plumbing is a standing question for the author and not three independent "
            "judgment calls. That claim is cited rather than asserted, because a "
            "reviewer was right that it cannot be checked from this repository as it "
            "stands: the other two adapters are not on the default branch yet. They are "
            "the ZotSeek seat, `adapters/zotseek.py:612` at merge request 236 head "
            "1eb1254, and the Beaver seat, `adapters/beaver.py:196` at merge request "
            "237 head 9af07d9. The two adapters that ARE on the default branch, for "
            "targets that speak stdio rather than HTTP, carry no port literal at all — "
            "which is the reviewer's observation and is consistent with this: the "
            "deviation belongs to the architecture class that has a listening socket, "
            "not to the roster as a whole.\n\n"
            "A THIRD preference, app.update.enabled=false, was written by an earlier "
            "version of this adapter and has been REMOVED as a genuine non-default "
            "option — see the note on HARNESS_PREFS; on this seat the application's own "
            "update check is inside R10's clause, not beside it. Re-measured after the "
            "removal, the subject arm's counts are unchanged, so the suppression had "
            "not in fact biased this run; the removal stands on the contract and not on "
            "that number.\n\n"
            ...
            "bench/results/0583-zotero-6012/r10-baseline.json. Over a 75 s dwell: "
            "virgin-isolated 0 off-machine / 430 name lookups, warmed-isolated 0 / 60, "
            "and the discriminating control virgin-shared 423 / 82, which is what says "
            "the instrument works here. Beside it, the lane lead's Zotero 10.0.1 cells "
            "on the same machine and driver: 0 / 426 virgin, 0 / 60 warmed, 456 / 80 "
            "shared. THE PULL REQUEST ADDS NO MEASURABLE EGRESS — the two builds' "
            "figures are within a few lookups of each other at both arena states, and "
            "both attempt zero off-machine connections. That is an observation about "
            "two runs of two builds and not an arithmetic claim; nothing is subtracted "
            "anywhere, in code or by hand.\n\n"
            "THE ARENA STATE IS PART OF THE MEASUREMENT, and the sevenfold gap between "
            "the two isolated cells is why it is stated rather than assumed: this "
            "adapter runs against a VIRGIN profile and data directory, created fresh "
            "inside the arena on every construction, never seeded and never cleared "
            "afterwards — the expensive end of the range, because a first run does "
            "update, blocklist and first-run work a warmed one does not.\n\n"
            "THE WINDOWS DIFFER AND THE NUMBERS ARE THEREFORE NOT COMPARABLE ACROSS "
            "THEM. The baseline cells above run a fixed 75 s dwell. The egress "
            "assertion's subject arm runs this adapter's own lifecycle — readiness plus "
            "a settle, then the two offered verbs, then the stop — which is roughly a "
            "third of that, and it reads 0 off-machine and about 40 name lookups, every "
            "one of them to the stub resolver on loopback. A smaller number over a "
            "shorter window is not less egress; it is less time. Read the subject arm "
            "against the clause, which is about whether anything is attempted, and read "
            "the cells against each other."
        ),
        unsupported={
            "uninstall": (
        ...
        not_derived_state=(
            (
                arena / HOST_LOG,
                "READ A GREEN RESIDUE VERDICT ON THIS TARGET AS EXACTLY THIS, and the "
                "statement belongs where a reader of the artifact meets it: nothing "
                "appeared in the arena outside the seven declared roots and the three "
                "desktop entries exempted below. Measured across two runs, the exempted "
                "groups are STABLE at 116 files — 107 the office suite's configuration, "
                "1 the sound server's, 8 inside .cache — plus 1 for this log; every "
                "other file created (963 and 953 on the two runs, the difference living "
                "in the toolkit's own cache, which is a DECLARED root) falls under a "
                "declared root. Read the check's own created_count for a run's total "
                "rather than a figure written here, which is what varies.\n\n"
                "What could still fail it: a write at the top of the sandbox "
                "HOME, a new directory under .config, a new top-level entry in the "
                "arena, or anything outside the arena's own tree. What could not: a new "
                "file inside .cache. A green here is not a claim that this target wrote "
                "nowhere else on the machine.\n\n"
                "This entry itself is the HARNESS's own instrument, not the target's "
                "state. The arena is "
            ...
            (
                ...
                "counted on real runs: .cache/fontconfig, 3 files, the "
                ...
                "cache, whose name embeds this machine's own id. EIGHT FILES, the same "
                "eight on both runs measured. The two "
                "subtrees here that are the target's, .cache/mozilla (100 and 110 files "
                "on those runs) and "
