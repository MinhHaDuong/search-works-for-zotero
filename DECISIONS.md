...
**2026-09-01 — the baseline-bump trigger is ratified, with (b) sharpened.**
The author's ruling on the awaiting entry ticket 0504 raised the day before;
the proposal's full argument is in that entry's text, preserved in this file's
history at the merge of the re-read (PR #128). Three triggers stand, any one
sufficient. (a) The acceptance harness exists (ticket 0032), so re-reading the
standing page is one run rather than twenty-four judgements. (b) **The page is
deliberately offered upstream, or cited in any outgoing text** — sharpened
from "faces outward", because this repository is public and the maintainer
already reads it, so the unqualified wording is always weakly true and would
let any session argue the trigger fired whenever convenient; deliberate offer
and citation are events with authors and dates, which is what a trigger needs.
(c) A re-read establishes that a row's `delivered` verdict is wrong rather
than merely dated.

Trigger (c) has already fired: the v1.12.0 re-read
(`verification/UPSTREAM-1.12.0-REREAD.md`) found the page under-reporting —
`none` where R16 is kept, a filing named as in flight that merged. The
ruling's immediate effect is therefore that ticket 0520 is authorized: its
`needs-human` label comes off with this entry, and its machine gates remain
...
as what it tests, and the per-question reuse ceiling replaced with a
per-document reuse floor.** Two follow-on corrections to the same-day
reframe above. First, legibility: the ≈97 table's dimension names
("Structural marks", "R33 probe shapes") didn't say what each row actually
probes; every row now carries a plain statement of what it tests plus a
worked example ("Find passage in a table cell", "Find passage after char
600000", "Find passage in VN content from an EN query"). Second, coverage:
naming concrete examples surfaced real sub-cases the abstract labels had
compressed away, so the count grows to 200 — but not by crossing every
dimension against the full nine-language roster, since sourcing
structural-mark-bearing (table/footnote/caption) documents specifically in
Arabic, Russian, Chinese, German, Hindi and Spanish would be a materially
larger lift than the SHOULD-language row already commits to, and this
ticket's own sourcing log already flags that tier as the hard,
sometimes-infeasible part. The cross-cut stops at the 3 MUST-tier languages
(EN/FR/VI) already committed for sourcing; a `languages` column on the tally
table makes this multiplier explicit per row instead of folding it silently
into q/sub-case — and one row (Intersections) is inherently about script
already, so its own sub-cases name a script directly and the multiplier
stays at 1 rather than double-counting.

The reuse bound above (a question anchors to at most 4 documents before a
distinct source is required) is retracted, and the new bound lands on the
other axis, not its inverse: a document must anchor at least 5 questions
before it's "spent," no ceiling on how many. These constrain different sides
of the question↔document relation, so this is a replacement, not a strict
inversion. Assuming each question anchors to one primary document — the
common case, not universal — 200 questions at a ≥5-reuse floor need at most
~40 distinct documents (200/5); a question anchoring several documents at
once only adds incidences, so it loosens this bound rather than breaking it.
Either way the practical effect is the one intended: concentrating cost in
...
- **Whether zoteus serves its own extraction in place of a platform full-text
  ...
  ticket 0480).** The population is now measured rather than estimated, and
  the measurement moves the question rather than settling it.
  ...
  **8211 carry no form feed** and were therefore written by the extractor
  ...
  genuinely single-page). Two text-quality signals the classifier did not use
  ...
  zoteus returns is no longer the text Zotero holds, for 8211 attachments.
  ...
  and two generations never mix unlabeled (the chunker key already carries the
  ...
  consent does not scale to 8211. If the author rules against serving our own
  ...
  One experiment is named and not run: nothing re-extracted a known document
  with the current extractor and watched a form feed appear. The
  cross-tabulation is circumstantial, the fixture control in
  `tests/test_fulltext_quality_census.py` proves the classifier reacts, and
  neither is that experiment. It is one item through a plugin-commanded
  reindex and it should precede any campaign.
