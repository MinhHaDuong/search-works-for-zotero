...
def test_bytecode_under_a_subdirectory_is_not_scanned(tmp_path):
    """Importing `bench/fixtures/fetch_recipe.py` from a test writes bytecode beside
    it, not under `bench/__pycache__/`. A prefix-only skip read that as an authored
    undecodable file and turned the gate red (2026-09-02)."""
    repo = build(tmp_path, {"bench/fixtures/fetch_recipe.py": "x = 1\n"})
    cache = repo / "bench" / "fixtures" / "__pycache__"
    cache.mkdir()
    (cache / "fetch_recipe.cpython-314.pyc").write_bytes(b"\x92\x00undecodable")
    assert cm.run(repo) == 0


def test_an_authored_undecodable_file_is_still_a_finding(tmp_path):
    """The control for the exemption above: the same bytes outside `__pycache__`."""
    repo = build(tmp_path, {"bench/fixtures/fetch_recipe.py": "x = 1\n"})
    (repo / "bench" / "fixtures" / "blob.bin").write_bytes(b"\x92\x00undecodable")
    assert cm.run(repo) == 1
...
def test_the_data_exemption_does_not_extend_to_siblings_by_prefix():
    ...
    the guard: `results/` is data whose cells must name their model, and
    `__pycache__` is generated bytecode of source already scanned. The second is
    matched as a path component rather than as the prefix `bench/__pycache__/`,
    because a test importing a script under a subdirectory writes its bytecode
