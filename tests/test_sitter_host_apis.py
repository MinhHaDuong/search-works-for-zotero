...
asks them whether the entry is there.

The mapping, verified on 2026-09-08 against ``/opt/zotero7`` by listing both
jars rather than by reading documentation:
...
#: Deliberately permissive on the right: it stops at a quote, whitespace, or a
#: closing bracket, so it reads the literal out of `require('resource://…')` and
#: out of a template string alike.
...
def resource_roots(install: Path) -> dict[str, tuple[str, str]]:
    ...
    for host, target in aliases.items():
        resolved = resolve_in(target, roots)
...
def resolve_in(url: str, roots: dict[str, tuple[str, str]]) -> tuple[str, str] | None:
    ...
    match = RESOURCE_URL.fullmatch(url)
    if match is None:
        return None
    ...
    if root is None:
        return None
    role, prefix = root
    return role, prefix + path


def named_resource_urls(tree: Path) -> dict[str, list[str]]:
    """Every `resource://` literal in the tree, mapped to where it is written."""
    ...
    for path in sorted(tree.rglob('*')):
        ...
        for number, line in enumerate(path.read_text(encoding='utf-8').splitlines(), 1):
            for match in RESOURCE_URL.finditer(line):
                ...
                sites.setdefault(match.group(0), []).append(site)
    return sites
...
def unanswered(urls: dict[str, list[str]], install: Path) -> list[str]:
    ...
    for url, sites in sorted(urls.items()):
        ...
        if resolved is None:
            problems.append(f'{url} ({where}): no resource host by that name in {install}')
            continue
        role, entry = resolved
...
def test_absent_url_is_not_extracted_from_prose():
    """The historical string is gone from the source, and prose does not fake it.

    `bootstrap.js` still discusses Fluent in several comments. If the extraction
    matched those, the control below would redden for the wrong reason and the
    real set would carry a phantom entry.
    """
...
def test_every_named_resource_exists_in_the_installed_zotero():
    ...
    assert not problems, 'host APIs the sitter names that Zotero does not have:\n' + '\n'.join(
        problems
    )
...
def test_guard_reddens_on_the_historical_absent_module(tmp_path):
    ...
    """
    install = require_install()
    real = sorted(named_resource_urls(PLUGINS))
    ...
    assert len(problems) == 1, f'expected only the Fluent import to redden, got: {problems}'
    assert HISTORICAL_ABSENT in problems[0]
    assert 'bootstrap.js:1' in problems[0], 'the guard names where the defect is written'
...
def test_the_zotero_host_is_derived_not_assumed():
    ...
    install = require_install()
    roots = resource_roots(install)
    assert 'zotero' in roots, f'{install} declares no resource://zotero/ alias'
